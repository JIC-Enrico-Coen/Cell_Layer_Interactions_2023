function [m,result] = gpt_utriculariathirdbladesmissing_20230319( m, varargin )
%[m,result] = gpt_utriculariathirdbladesmissing_20230319( m, varargin )
%   Morphogen interaction function.
%   Written at 2023-03-20 10:56:31.
...
% alter the set of nodes.
    ...
    if (Steps(m)==0) && m.globalDynamicProps.doinit
        ...
        m.cellbulkmodulus(:) = ones( getNumberOfFEs(m), 1 );
...
% In this section you may modify the mesh in any way whatsoever.
    if Steps(m)==0
        % Code to delete the middle third of the blades.
        % Not needed since the initial state now has the middle third
        % already cut away. Note that the code here will not quite work
        % anyway as the definition of id_blades has changed since it was
        % written.
%         ringheights = uniquetol( m.FEnodes(:,3), ABSTOL );
%         EPIDERMIS_THICKNESS = ringheights(3) - ringheights(1);
%         numRings = (length(ringheights)-1)/2;
%         ringsToDelete = floor(numRings/3);
%         if mod( numRings-ringsToDelete, 2 )==1
%             ringsToDelete = ringsToDelete - 1;
%         end
%         maxringheight = ringsToDelete * EPIDERMIS_THICKNESS/2;
%         feHeight = perVertextoperFE( m, m.FEnodes(:,3) );
%         bladeness = perVertextoperFE( m, id_blades_p );
%         fesToDelete = (abs(feHeight) < maxringheight) & (bladeness > 0);
%         m = leaf_deleteElements( m, fesToDelete );
    end
